...
test("non-JSON responses include status and body preview", async () => {
    const fetchImpl = async () => (new Response("not json", { status: 502, statusText: "Bad Gateway" }));
    const client = new LlmWikiApiClient({ fetchImpl });
...
test("API errors include status and server message", async () => {
    const fetchImpl = async () => (new Response(JSON.stringify({ ok: false, error: "Unauthorized" }), { status: 401 }));
    const client = new LlmWikiApiClient({ fetchImpl });
