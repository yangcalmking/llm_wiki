...
const server = new Server({ name: "llm-wiki", version: VERSION }, { capabilities: { tools: {}, resources: {} } });
...
server.setRequestHandler(ListResourcesRequestSchema, async () => {
    ...
    const projectsResp = await client.projects().catch(() => ({ projects: [], currentProject: null }));
    ...
    try {
        ...
        if (uri.kind === "files") {
            const response = await client.files(uri.projectId, {
                root: enumArg(uri.params.root, ["wiki", "sources", "all"], "wiki"),
        ...
        if (uri.kind === "reviews") {
            const reviews = await client.reviews(uri.projectId, {
                status: enumArg(uri.params.status, ["unresolved", "resolved", "all"], "unresolved"),
...
function parseLlmWikiUri(raw) {
    ...
    if (segments.length >= 2 && segments[0] === "projects") {
        ...
        if (rest[0] === "files") {
            if (rest.length === 1)
                return { kind: "files", projectId, params };
            return { kind: "file", projectId, filePath: rest.slice(1).join("/"), params };
        }
        if (rest[0] === "reviews" && rest.length === 1)
            return { kind: "reviews", projectId, params };
        if (rest[0] === "graph" && rest.length === 1)
            return { kind: "graph", projectId, params };
        if (rest[0] === "search" && rest.length === 1)
            return { kind: "search", projectId, params };
    }
    return null;
}
        ...
        switch (request.params.name) {
            ...
            case "llm_wiki_files": {
                ...
                const response = await client.files(projectId(args), {
                    root: enumArg(args.root, ["wiki", "sources", "all"], "wiki"),
            ...
            case "llm_wiki_reviews": {
                ...
                const reviews = await client.reviews(projectId(args), {
                    status: enumArg(args.status, ["unresolved", "resolved", "all"], "unresolved"),
...
async function assertMcpEnabled() {
    ...
    if (health.mcpEnabled === false) {
        throw new McpError(ErrorCode.InvalidRequest, "LLM Wiki MCP access is disabled. Enable Settings -> API + MCP -> Enable MCP access in the desktop app.");
    }
}
...
function pickStringArg(args, names) {
...
function formatFileTree(files, truncated = false) {
    if (files.length === 0)
...
function formatSearchResults(query, search) {
...
function formatReviews(response) {
    ...
    if (reviews.length === 0)
        return `No ${response.status} review items found.`;
    const lines = [
        "# Review items",
        "",
    ...
    reviews.forEach((review, index) => {
        ...
        if (review.description)
            lines.push(`Description: ${review.description}`);
        const optionSummary = formatReviewOptions(review);
        if (optionSummary)
            lines.push(`Options: ${optionSummary}`);
        lines.push("");
    });
    return lines.join("\n");
}
function formatReviewOptions(review) {
